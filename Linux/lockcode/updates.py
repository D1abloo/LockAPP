...
def _parse_release(value: dict) -> Release | None:
    ...
    for asset in value.get("assets", []):
        ...
        parsed = urlparse(url)
        if ("linux" in name.casefold() and name.endswith(".deb")
                and parsed.scheme == "https" and parsed.netloc == "github.com"
                and parsed.path.startswith("/D1abloo/LockAPP/releases/download/")
                and re.fullmatch(r"sha256:[0-9a-f]{64}", digest)):
            return Release(version, url, digest.removeprefix("sha256:"), int(asset.get("size", 0)))
    return None
...
def install_package(package: Path, report: Progress) -> None:
    ...
    process = subprocess.Popen(
        ["pkexec", "/usr/bin/apt-get", "-o", "APT::Status-Fd=1",
    ...
    for line in process.stdout:
        if progress := _apt_progress(line):
            report(*progress)
    result = process.wait()
...
def apply(release: Release, report: Progress) -> None:
    with tempfile.TemporaryDirectory(prefix="lockcode-update-") as directory:
        package = Path(directory) / f"lockcode-linux_{release.version}_all.deb"
